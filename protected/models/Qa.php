...
class Qa extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('question, answer, create_uid, create_time', 'required'),
			array('is_rec, is_show', 'numerical', 'integerOnly'=>true),
			array('question', 'length', 'max'=>150),
			array('orderid, create_uid', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, question, answer, is_rec, orderid, is_show, create_uid, create_time', 'safe', 'on'=>'search'),
		);
	}

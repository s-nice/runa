...
class Reservation extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('gender, name, phone, create_time, source', 'required'),
			array('province, city', 'safe'),
			array('gender', 'numerical', 'integerOnly'=>true),
			array('name, province', 'length', 'max'=>30),
			array('phone', 'length', 'max'=>20),
			array('city', 'length', 'max'=>60),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, gender, name, phone, province, city, create_time', 'safe', 'on'=>'search'),
		);
	}
